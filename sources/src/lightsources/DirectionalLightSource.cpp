 ...
 */
DirectionalLightSource::DirectionalLightSource(const Spectrum& spectrum, const Real& power, const Vector& direction, const Vector& u, const Vector& v, const Point& o)
: _direction(direction), _spectrum(spectrum), _lightPower(power), _u(u), _v(v), _o(o)
{
  _direction.normalize();
  _spectrum.normalizePower();
  _powerFactor = _lightPower/(u.norm()*v.norm());
}
 ...
 */
void DirectionalLightSource::getIncidentLight(const Point& receiver, const LightVector& reemited, std::vector<LightVector>& incidents)
{
  LightVector lightdata;

  //Building geometric part of this light data 
  lightdata.setRay(receiver, _direction);

  lightdata.setDistance(1000);

  if(_direction[2]<1.0 && _direction[2]>-1.0)
 ...
 */
Real DirectionalLightSource::getPower()
{
  return _lightPower;
}
 ...
 */
void DirectionalLightSource::getEmittedLight(const Basis& localBasis, const Point2D& surfaceCoordinate, LightVector& emitted)
{
  emitted.clear();
 ...
 */
void DirectionalLightSource::getRandomPhoton(MultispectralPhoton& photon)
{
  Real x = rand()/(Real)RAND_MAX;
  Real y = rand()/(Real)RAND_MAX;

  photon.position[0] = _o[0] + x * _u[0] + y*_v[0];
  photon.position[1] = _o[1] + x * _u[1] + y*_v[1];
  photon.position[2] = _o[2] + x * _u[2] + y*_v[2];
  photon.direction = _direction;
