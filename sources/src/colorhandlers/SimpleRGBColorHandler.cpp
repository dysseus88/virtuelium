 ...
 */
void SimpleRGBColorHandler::lightDataToRGB(const LightVector& lightdata, Pixel& color)
{
  Real rad = 50*(lightdata[2].getRadiance());
  color[0] = (rad<0)?0 : rad;
  rad =  50*(lightdata[1].getRadiance());
  color[1] = (rad<0)?0 : rad;
  rad =  50*(lightdata[0].getRadiance());
  color[2] = (rad<0)?0 : rad;
}
