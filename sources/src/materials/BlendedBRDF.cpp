 ...
 */
BlendedBRDF::BlendedBRDF(const std::vector<Material*>& materials, const std::vector<Real>& factors)  
: Material(hasDiffuseMaterial(materials), hasDiffuseMaterial(materials)), _materials(materials), _factors(factors)
{
  //Nothing to do more
}
 ...
 *   here).
 */
void BlendedBRDF::getDiffuseReemited(const Basis& localBasis, const Point2D& surfaceCoordinate, const LightVector& incidentLight, LightVector& reemitedLight)
{
  LightVector tmp;
  reemitedLight.clear();
 ...
 *   here).
 */
void BlendedBRDF::getSpecularReemited(const Basis& localBasis, const Point2D& surfaceCoordinate, const LightVector& incidentLight, LightVector& reemitedLight)
{
  LightVector tmp;
  reemitedLight.clear();
 ...
 */
void BlendedBRDF::getSpecularSubRays(const Basis& localBasis, const Point2D& surfaceCoordinate, LightVector& reemitedLight, std::vector<LightVector>& subrays)
{
  for(unsigned int i=0; i<_materials.size(); i++)
 ...
 */
bool BlendedBRDF::bouncePhoton(const Basis& localBasis, const Point2D& surfaceCoordinate, MultispectralPhoton& photon, bool& specular)
{
  Real factorsum=0;
  for(unsigned int i=0; i<_materials.size(); i++)
    factorsum+=_factors[i];

  for(unsigned int i=0; i<_materials.size(); i++)
  {
    if(rand() < _factors[i]*RAND_MAX/factorsum)
    {
 ...
 */
void BlendedBRDF::getRandomDiffuseRay(const Basis& localBasis, const Point2D& surfaceCoordinate, LightVector& reemitedLight, unsigned int nbRays, std::vector<LightVector>& subrays)
{
  int nbDiffuseMat=0;
  for(unsigned int i=0; i<_materials.size(); i++)
 ...
 */
bool BlendedBRDF::hasDiffuseMaterial(const std::vector<Material*>& materials)
{
 ...
 */
bool BlendedBRDF::hasSpecularMaterial(const std::vector<Material*>& materials)
{
  for(unsigned int i=0; i<materials.size(); i++)
...
void BlendedBRDF::getDiffuseReemitedFromAmbiant(const Basis& localBasis, const Point2D& surfaceCoordinate, LightVector& reemitedLight, const Spectrum& incident)
{
  LightVector tmp;
  reemitedLight.clear();
