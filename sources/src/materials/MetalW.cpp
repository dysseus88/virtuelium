...
//! @version 5.0.0
//! @date 2013
//! @details This file implements the class declared in MetalW.hpp:
//!  @arg MetalW
//! @todo 
//! @remarks 
//!
...
#include <physics/WardRoughnessFormula.hpp>
////////////////////////////////////////////////////////////////////////////////
MetalW::MetalW(std::vector<const Medium*>& media_stack, 
                     Real beck_mi, Real beck_mj)
    : Material(true, false) {

  VrtLog::Write("Entering MetalW::MetalW...");
  ...
  for(unsigned int iOi = 0; iOi < kSAMPLES; iOi++) {
    ...
    for(unsigned int i = 0; i < GlobalSpectrum::nbWaveLengths(); i++) {
      if(!(R12p[iOi][i] < Real(1.0) + kEPSILON)) {
        VrtLog::Write("(MetalW::MetalW) La réflectance 1-2 P de la \
bande %d vaut (%f) pour l'angle d'incidence %f°", 
                      i, R12p[iOi][i], (Oi * 180.0 / M_PI));
      }
      if(!(R12s[iOi][i] < Real(1.0) + kEPSILON)) {
        VrtLog::Write("(MetalW::MetalW) La réflectance 1-2 S de la \
bande %d vaut (%f) pour l'angle d'incidence %f°", 
                      i, R12p[iOi][i], (Oi * 180.0 / M_PI));
      }
      if(!(T12p[iOi][i] < Real(1.0) + kEPSILON)) {
        VrtLog::Write("(MetalW::MetalW) La transmittance 1-2 P de la \
bande %d vaut (%f) pour l'angle d'incidence %f°", 
                      i, R12p[iOi][i], (Oi * 180.0 / M_PI));
      }
      if(!(T12s[iOi][i] < Real(1.0) + kEPSILON)) {
        VrtLog::Write("(MetalW::MetalW) La transmittance 1-2 S de la \
bande %d vaut (%f) pour l'angle d'incidence %f°", 
                      i, R12p[iOi][i], (Oi * 180.0 / M_PI));
      }           
    }	
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getSpecularReemited(const Basis& localBasis, 
                                    const Point2D& surfaceCoordinate, 
                                    const LightVector& incidentLight, 
                                    LightVector& reemitedLight) {
  VrtLog::Write("Entering MetalW::getSpecularReemited...");
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getSpecularSubRays(const Basis& localBasis, 
                                   const Point2D& surfaceCoordinate, 
                                   LightVector& reemitedLight, 
                                   std::vector<LightVector>& subrays) {
  VrtLog::Write("Entering MetalW::getSpecularSubRays...");
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getDiffuseReemited(const Basis& localBasis,
	                                 const Point2D& surfaceCoordinate, 
                                   const LightVector& incidentLight,
                                   LightVector& reemitedLight) {
  VrtLog::Write("Entering MetalW::getDiffuseReemited...");
  ...
  Real beckmann = WardRoughnessFormula::WardDistribution(localBasis.k, 
                                                         -incident, 
                                                         -view,
                                                         microNormal,
                                                         m_mi,
  ...
  VrtLog::Write("m = (%f, %f), beckmann = %f", m_mi, m_mj, beckmann);
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getRandomDiffuseRay(const Basis& localBasis, 
	                                  const Point2D& surfaceCoordinate, 
                                    LightVector& reemitedLight, 
                                    unsigned int nbRays, 
                                    std::vector<LightVector>& subrays) {
  VrtLog::Write("Entering MetalW::getRandomDiffuseRay...");
  Vector view = reemitedLight.getRay().v;
  view.mul(-1.0);
  ...
  for(unsigned int i=0; i<nbRays; i++)  {
    Vector dir;
    Real weight;
    WardRoughnessFormula::getWardRandomRay(localBasis, view, 
                                           m_mi, m_mj, weight, dir);
...
////////////////////////////////////////////////////////////////////////////////
bool MetalW::bouncePhoton(const Basis& localBasis,
	                           const Point2D& surfaceCoordinate, 
  ...
  WardRoughnessFormula::reflect(normal, photon.direction, m_mi, m_mj);
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getReflectance(const Real& cosOi, const int& index, 
...
////////////////////////////////////////////////////////////////////////////////
void MetalW::getDiffuseReemitedFromAmbiant(const Basis& localBasis,
	                                            const Point2D& surfaceCoordinate, 
                                              LightVector& reemitedLight, 
