...
#ifndef _VARNISHEDLAMBERTIANBRDF_HPP
#define _VARNISHEDLAMBERTIANBRDF_HPP
...
class VarnishedLambertianBRDF : public Material{
public :
   ...
   */
  VarnishedLambertianBRDF(std::vector<Spectrum> R12p, std::vector<Spectrum> R12s, std::vector<Spectrum> T12p, std::vector<Spectrum> T12s, std::vector<Spectrum> R21p, std::vector<Spectrum> R21s, const Spectrum& n, const Spectrum& k, Real thickness, const Spectrum& R23); 
   ...
   */
  VarnishedLambertianBRDF(std::vector<Spectrum> R12p, std::vector<Spectrum> R12s, std::vector<Spectrum> T12p, std::vector<Spectrum> T12s, std::vector<Spectrum> R21p, std::vector<Spectrum> R21s, const Spectrum& n, const Spectrum& k, Real thickness, const Spectrum& R23, const Spectrum& T23); 

  /**
   * Virtual destructor.
   */
  virtual inline ~VarnishedLambertianBRDF();
...
private :
  ...
  std::vector<Spectrum> _Td; //Index coded as Oi*_samples + Ov
   ...
   */
  void getDiffuseReflectance(const Real& cosOi, const Real& cosOv, const int& index, Real& R);
   ...
   * @param Rpara : the parallel polarised computed Reflectance will be placed here
   */
  void getDiffuseTransmittance(const Real& cosOi, const Real& cosOv, const int& index, Real& T);
...
inline VarnishedLambertianBRDF::~VarnishedLambertianBRDF()
{
  //Nothing to do
}

#endif //_VARNISHEDLAMBERTIANBRDF_HPP
