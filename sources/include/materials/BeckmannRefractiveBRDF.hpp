...
#ifndef _BeckmannRefractiveBRDF_HPP
#define _BeckmannRefractiveBRDF_HPP
...
class BeckmannRefractiveBRDF : public Material{
public :
  /**
   * Constructor
   * spectrum : the normal incident reflection spectrum.
   */
  BeckmannRefractiveBRDF(const std::vector<Spectrum>& RPara, const std::vector<Spectrum>& ROrth, std::vector<Spectrum> TPara, std::vector<Spectrum> TOrth, Spectrum indices, Real roughness);  

  /**
   * Virtual destructor.
   */
  virtual inline ~BeckmannRefractiveBRDF();
...
private :
  ...
  void getDiffuseReflected(const Vector normal, const LightVector& incidentLight, LightVector& reemitedLight);
  /**
   * Compute the diffusely reemited light data and place the result into reemited.
   */
  void getDiffuseTransmitted(const Vector normal, const LightVector& incidentLight, LightVector& reemitedLight);
...
inline BeckmannRefractiveBRDF::~BeckmannRefractiveBRDF()
{
  //Nothing to do
}

#endif //_BeckmannRefractiveBRDF_HPP
