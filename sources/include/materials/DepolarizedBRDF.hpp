...
#ifndef _DEPOLARIZEDBRDF_HPP
#define _DEPOLARIZEDBRDF_HPP
...
class DepolarizedBRDF : public Material{
public :
  /**
   * Constructor
   * material : the material to depolarise (reflected light will not be polarised).
   */
  DepolarizedBRDF(Material* material);  

  /**
   * Virtual destructor.
   */
  virtual inline ~DepolarizedBRDF();
...
inline DepolarizedBRDF::~DepolarizedBRDF()
{
  if(_material!= NULL)
    delete _material;
  _material = NULL;
