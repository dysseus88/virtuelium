...
/**
 * This interface define the Material. A material will have to rule the light
 * interactions. We need furthermore some definitions.
 *
 * There is two type of reflexion : specular and diffuse. This is a infographic
 * point of view, not a physic one. A specular reflexion will give one ray (or a
 * constant number of ray) for one incoming ray, a diffuse reflexion will scatter
 * the light over a solid angle for an incident ray.
 *
 * The usage of the different kind of contribution is depend of the renderer.
 * Some renderer (SimpleRenderer for exemple) will only use the getDiffuseReemited(),
 * getSpecularReemited(), getEmited() and getSpecularSubRays() methods. Typically the
 * bouncePhoton() method is designed for use with photon mapping, and getRandomDiffuseRay()
 * for Monte Carlo rendering of the diffuse contribution.
 *
 * Note : This interface will probably be changed in the futur...
 *
...
public :
   ...
   */
  Material(bool diffuse, bool specular);
...
inline Material::Material(bool diffuse, bool specular)
 ...
 * @return : true if this material has a diffuse component.
 */
inline bool Material::isDiffuse() const
{
  return _diffuse;
}
 ...
 * @return : true if this material has a specular component.
 */
inline bool Material::isSpecular() const
{
  return _specular;
}
