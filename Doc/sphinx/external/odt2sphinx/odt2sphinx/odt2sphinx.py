...
class RstFile(object):
    ...
    def insert_rows_spanned_cells(self):
        ...
        if len(self.current_table) > 1:
            for i in xrange(len(row), len(self.current_table[0])):
                last_line_i = -2
                separators = 0
    ...
    def start_cell(self, columns_span, rows_span):
        ...
        for i in xrange(1, columns_span):
    ...
    def end_table(self):
            ...
            for col_i, colw in enumerate(column_widths):
                if col_i in skip_columns:
                    ...
                    if col_i in group_last_columns:
                        if col_i + 1 in skip_columns or col_i + 1 == len(column_widths):
                            line += '|'
                        else:
                            line += '+'
                    else:
        ...
        for row_i, row in enumerate(rendered_cells):
            for line_i in xrange(0, row_heights[row_i]):
                line = '| '
                for cell_i, cell in enumerate(row):
                    if cell[0] is not None and len(cell[0]) > line_i:
                        line += cell[0][line_i] + ' '*(column_widths[cell_i]-len(cell[0][line_i]))
                    else:
            ...
            if row_i == separator_index:
                next_border_char = '='
            border = h_border(char=next_border_char, skip_columns_groups=skip_columns_groups, skip_nodes=skip_nodes)
            self.append(border)
    ...
    def getvalue(self):
        ...
        for fname in self.images.keys():
            ...
            if height:
                kw['height'] = height
            self.simpledirective('|%s| image' % fname.replace('.', '_'), 'images/' + fname, **kw)
...
class SphinxWriter(object):
    ...
    def add_image(self, fname, fcontent, width=None, height=None):
        self.current_file.add_image(fname, fcontent, width=width, height=height)
    ...
    def h1(self, text):
        ...
        self.current_file = RstFile()
        fname = text.replace(' ', '').replace('/', '').replace(':', '_').encode('ascii', 'ignore')
    ...
    def list_item(self, text=None):
        self.current_file.start_line('* ')
        if text is not None:
            self.current_file.append(text)
            self.current_file.append()
        else:
            self.current_file.start_bloc(2)
...
class ODTReader(object):
    ...
    def __init__(self, file, visitor, debug=False):
        self.debug = debug
    ...
    def _load_styles(self):
        ...
        for node in stylenodes:
            ...
            # Special case. We know that the "Source_xx_Text" styles are for source
            # code.
    ...
    def get_node_kind(self, node):
        ...
        if style and style_name not in self.style_map and style['parent-style-name'] in self.style_map:
    ...
    def handle_node(self, node):
        ...
        if node.tag == '{%(draw)s}frame' % self.ns:
            ...
            if len(children) == 1 and children[0].tag == '{%(draw)s}image' % self.ns:
                ...
                self.call_visitor('on_image', fname, fcontent, width=width, height=height)
            else:
                self.recurse_node(node)
        ...
        if node.tag == '{%(table)s}table-cell' % self.ns:
            self.start_visit('table_cell',
                columns_span=int(node.get('{%(table)s}number-columns-spanned'%self.ns, 1)),
                rows_span=int(node.get('{%(table)s}number-rows-spanned'%self.ns, 1)),
            )
            self.recurse_node(node)
    ...
    def iterate(self):
        nodes = self.xml_content.findall('{%(office)s}body/{%(office)s}text/*' % self.ns)
